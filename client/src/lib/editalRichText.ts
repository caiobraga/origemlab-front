    ...
    if (s[j] === "}") {
      ...
      for (const [k, vals] of merged.entries()) {
        if (vals.length === 1) {
          ...
          const strings = vals.filter((v: JsonValue) => typeof v === "string") as string[];
          const nonStrings = vals.filter((v: JsonValue) => typeof v !== "string");
          const mergedText = mergeUniqueParagraphs(strings);
          const extras = nonStrings.map((v: JsonValue) => JSON.stringify(v));
